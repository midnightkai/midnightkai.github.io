...
var chartData = {
  ...
  options: {
    title: {
      display: true,
      text: 'Apples, Lemons, and Oranges'
    },
    scales: {
      yAxes: [{
        ticks: {
...
function loadContent() {
  covidJsObj = localStorage.getItem("data");
  if(covidJsObj){
    covidJsObj = JSON.parse(covidJsObj);
    if((Date.now() - covidJsObj.retrieveDate) > (24 * 60 * 60 * 1000)){
      covidJsObj = null;
    }
  }
  ...
  xhttp.onreadystatechange = function() {
    if (this.readyState == 4 
      ...
      covidJsObj.retrieveDate = Date.now();
